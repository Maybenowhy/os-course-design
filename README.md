...
本项目选择课程设计要求中“调度与性能优化”方向下的 **并发性能优化实验** 作为提升部分。

该实验位于 `进程同步与并发控制` 模块中，运行程序后选择：
...
实验使用相同的总计数任务，对比三种实现方式：

- 单线程基线：单线程完成全部计数，作为基础耗时参照。
- 多线程全局锁计数：多个线程共同更新一个全局计数器，每次更新都使用互斥锁保护，能够保证结果正确，但锁竞争明显。
- 多线程局部汇总：每个线程先在本地变量中计数，线程结束后再汇总结果，减少共享写入和锁竞争。

程序会输出线程数、每线程循环次数、实际计数、期望计数和耗时。三组计数均应等于期望总计数；通常情况下，多线程全局锁计数耗时较高，多线程局部汇总耗时更低，可用于说明并发程序中临界区设计和锁竞争对性能的影响。
